...
class Classifier:
    ...
    def __init__(self, data="training_data.csv"):
        ...
        if os.path.exists(data):
            self.prev_data = pd.read_csv(self.trainingDataFile, dtype= {'amount':'float64'})
        else:
            ...
            for i, tx in enumerate(txs.getTransactions()):
                ...
                if self.has_no_category(tx, args):
                    ...
                    if text != guess and text != args.rules.default_expense:
                        ...
                        #print(tx)
                        just_numbers = re.sub("[^0-9\.-]", "", str(tx.postings[0].units))
                        df = pd.DataFrame(
                            {"date":tx.date,
                             "desc":StringUtils.strip_digits(tx.payee.upper()),
                             "amount": just_numbers,
                             "cat":[text]
                        })
                        ...
                        # save training data
                        df.to_csv(self.trainingDataFile, mode='a', header=False, index=False)
