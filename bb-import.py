...
def resolve_amount(row, args):
    ...
    if args.amount_pos_in != -99:
        return D(row[args.amount_pos_in].strip().replace(args.currency_sep, '.')) - \
            D(row[args.amount_pos].strip().replace(args.currency_sep, '.'))
...
def init_arg_parser():
    ...
    parser.add_argument('-b', '--beancount_file',
                        help='Target beancount file', required=True)
    parser.add_argument('-s', '--separator',
                        help='CSV file separator', default=',')
    parser.add_argument('-l', '--skip',
                        help='Number of lines to skip', default=1, type=int)

    # csv column indexes
    parser.add_argument('-d', '--date_pos', help='',
                        default=0, type=int)
    parser.add_argument('-p', '--payee_pos', help='',
                        default=3, type=int)
    parser.add_argument('-m', '--amount_pos', help='',
                        default=4, type=int)
    parser.add_argument('-q', '--amount_pos_in', help='',
                        default=-99, type=int)

    parser.add_argument('-i', '--account_pos', help='',
                        default=8, type=int)
    parser.add_argument('-c', '--currency_pos', help='',
                        default=5, type=int)
    parser.add_argument('-x', '--tx_type_pos', help='',
                        default=2, type=int)
    # end csv column indexes

    parser.add_argument('-o', '--date_format', help='')
    parser.add_argument('-t', '--currency_sep',
                        help='Currency decimal separator', default='.')
    ...
    parser.add_argument('-g', '--force_negative',
    ...
    with open(args.file) as csv_file:
        ...
        for row in csv_reader:
            ...
            try:
                ...
                if md5 not in tx_hashes:
                    ...
                    print(
                        'warning: a transaction with identical hash exists in the journal: ' + md5)
                    log_error(row)
                    hash_collision += 1
        ...
        if len(accounts) == 1 and transactions:
            ...
            if len(transactions) > 0:
                print("Expecting only one account in csv file, found: " +
                      str(len(accounts)))
